...
namespace Payneteasy {
	function trace($arg, string $prefix=''): void {
		if (is_object($arg))
		...
		foreach ($arg as $key => $value) {
			...
			elseif ($key == 'credit_card_number')
				$value = (strlen($value) > 4) ? str_repeat('*', strlen($value)-4).substr($value, -4) : $value;
	...
	class PneException extends \Exception {
		public function __construct(string $message, $a1=null, $a2=null, $hdr=null) {
	...
	class PneApi {
		...
		private function execute(string $action, array $data): array {
			$url = $this->gate . self::URL . $action . ($this->is_multicurr ? '/group/' : '/') . $this->end_point;
			...
			if ($result['type'] == 'validation-error')
				throw new PneException("Card processing returned error: '{$result['error-message']}'", $data, $result, [ 'URL' => $url ]);
